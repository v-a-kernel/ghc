...
Note [Unique in CtLoc]
~~~~~~~~~~~~~~~~~~~~~~
Heterogeneous equality contraints throw off Derived kind constraints.
See Note [Equalities with incompatible kinds] in TcCanonical. However,
given a Derived, there is no way to know what constraint it arose from.
The Unique in a CtLoc allows us to track this. This is important in
floatEqualities, where we must make sure not to float out any equality
whose derived kind equalities are insoluble. Otherwise, we get a
duplicate error message. The test case is (typecheck/should_fail/T7368).
...
data CtLoc = CtLoc { ctl_origin :: CtOrigin
                   ...
                   , ctl_uniq   :: Unique }          -- Note [Unique in CtLoc]
...
instance Uniquable CtLoc where
  getUnique = ctl_uniq
